...
if (!$cursos || !$usuarios || !$materias_disponibles) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="#"
                                            ...
                                            onclick="cargarDatosCurso(<?php echo $curso['curso_id']; ?>, '<?php echo htmlspecialchars($curso['curso_nombre']); ?>', '<?php echo htmlspecialchars($curso['materias']); ?>');">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="border p-2 rounded" style="max-height: 200px; overflow-y: scroll;">
                                    <?php while ($materia = $materias_disponibles->fetch_assoc()): ?>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="materia-<?php echo $materia['id']; ?>" name="materias[]" value="<?php echo $materia['id']; ?>">
                                            <label class="form-check-label" for="materia-<?php echo $materia['id']; ?>"><?php echo htmlspecialchars($materia['nombre']); ?></label>
                                        </div>
                                    <?php endwhile; ?>
                                </div>
                            </div>
...
<script>
    function cargarDatosCurso(id, nombre, materias) {
        ...
        document.querySelectorAll('[name="materias[]"]').forEach((checkbox) => {
            checkbox.checked = false;
        });

        // Marcar las materias asignadas al curso
        materias.split(',').forEach((materiaNombre) => {
            document.querySelectorAll('[name="materias[]"]').forEach((checkbox) => {
                if (checkbox.nextElementSibling.innerText.trim() === materiaNombre.trim()) {
